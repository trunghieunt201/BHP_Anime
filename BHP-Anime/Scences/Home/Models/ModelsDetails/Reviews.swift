...
public final class Reviews: NSCoding {
  ...
  public required init(json: JSON) {
    ...
    if let items = json[SerializationKeys.results].array { results = items.map { $0.object} }
  ...
  required public init(coder aDecoder: NSCoder) {
    ...
    self.results = aDecoder.decodeObject(forKey: SerializationKeys.results) as? [Any]
